...
comb.dta <- full_join(dta.sum, savings.dta.sum, by = "Month")

# Add incomes and expenditures from the two accounts
comb.dta <- comb.dta %>%
...
prev.total.income <- as.numeric(prev.dta.cumsum[prev.num.months, 2]) +
  as.numeric(prev.savings.dta.cumsum[prev.num.months.sav, 2])

prev.total.expenditure <- as.numeric(prev.dta.cumsum[prev.num.months, 3]) +
  as.numeric(prev.savings.dta.cumsum[prev.num.months.sav, 3])
