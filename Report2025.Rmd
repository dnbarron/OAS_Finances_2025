...
members_dta %>% kbl(format.args = list(big.mark = ",")) %>%
  kable_classic(full_width = FALSE, html_font = "Georgia") %>%
  row_spec(0:nrow_mbs, color = "#000080") %>%
  row_spec(nrow_mbs, bold = TRUE, color = "white", background = "black")%>%
  row_spec(0, bold = TRUE) # %>%
#  kable_styling(latex_options = "hold_position")

members_outcome <- as.numeric(members_dta[nrow_mbs, 2] - members_dta[nrow_mbs, 3])
```
...
```{r ya, eval=TRUE}
nrow_ya <- dim(ya_dta)[1]

ya_dta %>% kbl(format.args = list(big.mark = ",")) %>%
  kable_classic(full_width = FALSE, html_font = "Georgia") %>%
  row_spec(0:nrow_ya, color = "#000080") %>%
  row_spec(nrow_ya, bold = TRUE, color = "white", background = "black")%>%
  row_spec(0, bold = TRUE) # %>%
#  kable_styling(latex_options = "hold_position")

ya_outcome <- as.numeric(ya_dta[nrow_ya, 2] - ya_dta[nrow_ya, 3])
